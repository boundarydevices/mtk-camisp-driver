...
#include "imx214d2lmipiraw_Sensor.h"
#include "imx214d2l_ana_gain_table.h"
...
#define PFX "IMX214D2L_camera_sensor"
...
#define IMX214D2L_SUPPORTED_SENSOR_MODE_NUMBER 5

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX214D2L_SENSOR_ID,
	.checksum_value = 0xf61b7b7c,

	.pre = {
		.pclk = 240384000,  //record different mode's pclk
	...
	.sensor_mode_num = IMX214D2L_SUPPORTED_SENSOR_MODE_NUMBER,    //support sensor mode num
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[IMX214D2L_SUPPORTED_SENSOR_MODE_NUMBER] = {
	/* Preview */
	{4208, 3120, 0000, 0000, 4208, 3120, 2104, 1560,
	 0000, 0000, 2104, 1560, 0000, 0000, 2104, 1560},
	/* capture */
	{4208, 3120, 0000, 0000, 4208, 3120, 4208, 3120,
	 0000, 0000, 4208, 3120, 0000, 0000, 4208, 3120},
	/* video */
	{4208, 3120, 0000, 0000, 4208, 3120, 4208, 3120,
	 0000, 0000, 4208, 3120, 0000, 0000, 4208, 3120},
	/* hight speed video */
...
static kal_uint32 imx214d2l_ATR(struct subdrv_ctx *ctx, UINT16 DarkLimit, UINT16 OverExp)
{
...
{
	//Preview 2104*1560 24M MCLK 2lane 1200Mbps/lane
	// preview 30fps
	...
	write_cmos_sensor_8(ctx, 0x0340, 0x06);
	write_cmos_sensor_8(ctx, 0x0341, 0x40);
	...
	write_cmos_sensor_8(ctx, 0x0821, 0x60);
	...
	write_cmos_sensor_8(ctx, 0x0202, 0x06);
	write_cmos_sensor_8(ctx, 0x0203, 0x00);
...
{
	LOG_INFO("E! currefps:%u\n", currefps);
	// capture setting 4208*3120 1200Mbps/lane

	LOG_DBG("E 15fps setting\n");
	// full size 15fps
	write_cmos_sensor_8(ctx, 0x0100, 0x00);
	write_cmos_sensor_8(ctx, 0x0114, 0x01);
	write_cmos_sensor_8(ctx, 0x0220, 0x00);
	...
	write_cmos_sensor_8(ctx, 0x3A04, 0x70);
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(ctx, 0x0016) << 8) |
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INFO("IMX214D2L i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_DBG("IMX214D2L Read sensor id fail, write id:0x%x id: 0x%x\n",
				ctx->i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		LOG_ERR("Get IMX214D2L sensor id fail.");
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(imx214d2l_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)imx214d2l_ana_gain_table,
			sizeof(imx214d2l_ana_gain_table));
		}
		break;
...
const struct subdrv_entry imx214d2l_mipi_raw_entry = {
	.name = "imx214d2l_mipi_raw",
	.id = IMX214D2L_SENSOR_ID,
	.pw_seq = pw_seq,
